import * as querySerializer from 'query-string'
...
describe("thunk", () => {
  ...
  it("attemptCallRouteThunk calls thunk with same `dispatch` argument as in middleware chain", () => {
    ...
    // changed `toEqual(3)` to `toEqual(2)` and `calls[0][2]` to `calls[0][1]` to avoid ts erros about tuples
    expect(thunk.mock.calls[0].length).toEqual(2);
    expect(thunk.mock.calls[0][1].action).toMatchObject(action);
    expect(thunk.mock.calls[0][1].extra).toEqual("extra-arg");
  });
  ...
  it('store.getState().location.kind === "redirect" after awaiting thunk', async () => {
    ...
    global.window.SSRtest = true;
    const { store, history, thunk: ssrThunk } = setupThunk(
      "/second/bar",
      thunk
    );
    delete global.window.SSRtest;
